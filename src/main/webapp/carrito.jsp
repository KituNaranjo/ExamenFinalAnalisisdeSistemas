...
<main>
	...
	<table border="1">
        ...
        <c:forEach items="${producto.contenidoCarrito}" var="item" varStatus="status">
            <tr>
                <td>${status.index + 1}</td>
                <td>${item.nombre}</td>
                <td>${item.cantidad}</td>
                <td>${item.precio}</td>
                <td><a href="EliminarDelCarrito.jsp?indice=${status.index}" class="boton-enlace">Eliminar</a></td>
            </tr>
        </c:forEach>
    </table>
    <p>Total: ${producto.totalCarrito}</p>
    <a href="productos2.jsp" class="boton-enlace">Volver a la lista de productos</a><br>
  	</div>
	<br>
	<footer>
